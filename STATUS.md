...
Keeper `2026-09-11 11:41 CDT`: https://dkrics.github.io/oldfashioneddoctor/ serves Astro (cream/amber, covers, Drinks). Apex `oldfashioneddoctor.com` still Blogger A records `216.239.*`. `www` → `ghs.google.com`. Cusdis.com HTTP **521**. No Squarespace/Google session on the box browser (login walls — cookies show expired/absent SS_SESSION_ID; no Google SID/HSID). GitHub Pages `cname` null (preview mode). `gh` CLI authenticated as DKrics.
...
2. **Cusdis** App ID secret (hosted cusdis.com still HTTP **521** as of `2026-09-11 11:41 CDT`)
