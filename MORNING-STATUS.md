...
Keeper check `2026-09-16 06:55 CDT`: github.io Astro preview remains OK (HTTP 200; cream/amber branding, covers, Astro title/brand, Drinks nav; `/drinks/` HTTP 200). Apex still redirects 301 to `www`, then both serve Blogger/GSE over HTTPS (apex final HTTP 200; www HTTP 200); apex resolves to Blogger `216.239.32/34/36/38.21` and `www` to Google frontends. Hosted Cusdis remains HTTP **521** (abandoned); comments email form remains enabled in the deployed build (`web3forms` + `access_key` on post pages). GitHub Pages API still reports `cname: null`, `https_enforced: true`, and built workflow status; latest `site` deploy is green. No usable Squarespace/Google session on the box (no Squarespace auth cookies in available Chrome DBs; Google cookies lack SID/HSID/SSID). DNS remains Homey-owned. No material change vs 05:59 CDT; no WakeParent (before 7:30 CDT).
