...
Keeper `2026-09-17 12:06 CDT`: preview re-verified via curl HTTP 200 as Astro (Old Fashioned Doctor title/brand, cream/amber `#f4ead8`/`#d9c48a`/`#6b3e1a`, covers, Drinks nav); `/drinks/` HTTP 200. Apex redirects 301 to `www`, then both serve Blogger/GSE over HTTPS (Powered by Blogger; apex DNS `216.239.32/34/36/38.21`; www CNAME `ghs.google.com`). Hosted Cusdis remains HTTP **521** (abandoned); comments Web3Forms form remains enabled (`access_key` present on post pages). GitHub Pages API still reports `cname: null`, `https_enforced: true`, workflow built; latest `site` deploy green. No usable Squarespace/Google session on the box (ANONYMOUS_ID on login.squarespace.com; Google cookies lack SID/HSID/SSID). DNS remains Homey-owned (NS `ns-cloud-c*.googledomains.com`). No material change vs 11:15 CDT; no WakeParent (after 7:30 CDT).
